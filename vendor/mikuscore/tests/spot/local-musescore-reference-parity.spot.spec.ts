...
describe("Local parity: mscx vs reference musicxml", () => {
  ...
  itWithLocalFixture("converts local mscx fixture and requires zero semantic diffs", () => {
    ...
    console.log(`ref notes=${refEvents.length} cand notes=${candEvents.length}`);
    // eslint-disable-next-line no-console
    console.log(`diff pitch-only=${diffPitch.length} diff pitch+acc=${diffPitchAcc.length}`);
    if (diffPitch.length > 0) {
      // eslint-disable-next-line no-console
      console.log("pitch-only sample:\n" + diffPitch.slice(0, 40).join("\n"));
    }
    if (diffPitchAcc.length > 0) {
      // eslint-disable-next-line no-console
      console.log("pitch+acc sample:\n" + diffPitchAcc.slice(0, 40).join("\n"));
    }
